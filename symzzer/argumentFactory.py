...
class ArgumentFactory(object):
    ...
    def generateNewData(self, _funcName, kind):
        ...
        if kind == 0:
            ...
            self.testArgument = f'{{"from":"{self.fakeTransferUserName}", ' + \
                                ...
                                f'"quantity":{self.generateDataForSimpleType("asset")}, ' + \
                                f'"memo":{self.generateDataForSimpleType("string")}}}'      #随即生成quantity、memo
    ...
    def generateNewDataType(self, _funcName):
        self.testArgumentType = self.generateDataType(self.getStructDetail(self.getActionType(_funcName))) # type、struct
    ...
    def generateDataForSimpleType(self, structName):
        ...
        """generatinh ramdon value for simple arguments 为简单参数生成随机值
        Args:
        ...
        Returns:
            The data
        """
        ##通过解析JSON文件以提取每个参数的数据类型，为每个数据类型生成输入
