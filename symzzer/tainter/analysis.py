...
class AssetEOS:
    def __init__(self, val):
        ...
        self.precision = (self.symbol & 255) % 19 
...
class Analysis(object):
    ...
    def solve(self, location, currentCondition, conditionalTarget, symCondition):
        ...
        if conditionalTarget == -1:
            ...
            if currentCondition == 0:
                flag = 1
                e = simplify(symCondition == BitVecVal(1, 32)) # jump True
            else:
                flag = 0
                e = simplify(symCondition == BitVecVal(0, 32)) #  don't jump False
        ...
        #print('[-] pathConstratins : ',self.pathConstratins)
        return 
    ...
    def seedMining(self, fbTuple, mod):
        for val in mod: 
            ...
            if tp.endswith('[]'): 
                ...
                print("[-]cleosArgs : ",self.cleosArgs)              
    ...
    def arg2abiType(self, symbol, symExpr, tp, oriData):
        ...
        if  tp == 'asset':
            ...
            sym = list(aSym)
            for i in sym:
                if not (i.isalpha() and 'a' <= i <= 'z' or 'A' <= i <= 'Z'):
                    aSym = oriSym
                    break            
    ...
    def local(self, location,  op, localIndex, value):
        # symval = self.stack.peek().pop() CHANGED BY wx
        if op == "local.set":
