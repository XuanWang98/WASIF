...
def localPos(name):
    ...
    t = re.search(r"L(\d+)", name) #re.search()方法扫描整个字符串，并返回第一个成功的匹配。如果匹配失败，则返回None。
...
def buildArgs(instr, args, types):
    ...
    for t in types:
        if t == 'I32' or t == 'F32' or t == 'F64':
            ...
            arg1 = realArgs[idx]
            arg2 = realArgs[idx+1]
            val = (arg2 << 32) | arg1 #<<  :是左移动运算符,运算数的各二进位全部左移若干位，由 << 右边的数字指定了移动的位数，高位丢弃，低位补0。
            #x << y = x乘以2**y
            # | : 他会将数字转化为2进制，然后判断两个二进制数字。相同位数的数字有1则为1，没有1则为0
            idx += 2
        
        dataSize = tp2len(t)
        if t[0] == 'I':
            result = BitVecVal(val, dataSize)   #dataSize位，值为val的位向量
        else:  # Floatvim
            if dataSize == 32:
                result = FPVal(val, Float32())
            elif dataSize == 64:
